...
export default function GameScreen({ players, gameId, onRestart }) {
    // If drawing game selected, show placeholder until we build it
    if (gameId === 'drawing') {
        return (
            <div style={styles.container}>
                <h1 style={styles.title}>🎨 Draw &amp; Steal</h1>
                <p style={{ color:'rgba(255,255,255,0.5)', fontSize:22 }}>Coming soon…</p>
                <button style={styles.restartBtn} onClick={onRestart}>Back →</button>
            </div>
        )
    }

    // Default: tapping game
    return <TappingGame players={players} onRestart={onRestart} />
}

function TappingGame({ players, onRestart }) {
    ...
    useEffect(() => {
        ...
        setState('scores', { ...currentScores, [name]: (currentScores[name] ?? 0) + 1 })
        setState('times',  { ...currentTimes,  [name]: (currentTimes[name]  ?? 0) + reactionMs })
        setWinner(name)
        doAdvance(roundRef.current)
    }, [tapEvent])
        ...
        return (
            <div style={styles.container}>
              ...
              <span style={{ fontSize:28 }}>
                ...
                {fastestName && (
                    <p style={{ fontSize:22, color:'#FFD93D', margin:0 }}>⚡ Fastest overall: {fastestName}</p>
                )}
...
const styles = {
    ...
    promptTarget: { fontSize:72, lineHeight:1.1, textAlign:'center', color:'#fff' },
